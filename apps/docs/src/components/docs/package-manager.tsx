...
const packageManagers: PackageManager[] = [
  {
    ...
    label: "CLI",
    icon: (
      <CLIBoldIcon className="text-lg text-default-600 dark:text-default-400" />
    ),
  },
  {
    name: "pnpm",
...
export const PackageManagers = ({
  ...
  const availableManagers = packageManagers.filter((pm) =>
    Boolean(commands[pm.name]),
  );
  const [selectedManager, setSelectedManager] =
    React.useState<PackageManagerName>(
      ...
      >
        ...
        {availableManagers.map(({ name }) => (
          <TabsContent key={name} value={name}>
            <DynamicCodeBlock lang="bash" code={commands[name] as string} />
          </TabsContent>
        ))}
      </Tabs>
    </>
